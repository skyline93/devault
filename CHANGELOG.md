...
- **控制台（IAM）**：平台管理 **`/platform/users/new`**、**`/platform/users`**（调用 IAM **`/v1/platform/users`** 与 **`/v1/tenants/{id}/members`**；创建时客户端生成初始密码并 **`must_change_password: true`**）；**`/user/change-password`** 与登录门禁（**`localStorage`** **`devault_iam_must_change_password`** + **`devault_refresh_token`**；**`RequireSession`** 在待改密时重定向改密页）。OpenSpec 变更 **`openspec/changes/add-console-platform-user-management/`**；文档 **`website/docs/guides/web-console.md`**、**`console/README.md`**。
- **Agent 令牌与边端一键演示（控制面重构）**：迁移 **`a1b2c3d4e5f6`**（**`devault_agent_tokens`**、**`edge_agents.agent_token_id`**；移除 **`agent_enrollments`** / **`agent_pools`** / **`agent_pool_members`** 与 **`policies.bound_agent_pool_id`**）。REST **`/api/v1/agent-tokens`**（创建返回一次性 **`plaintext_secret`**，库内仅存 **`token_hash`**）；gRPC 全链路 **`Authorization: Bearer`** 校验令牌（租户范围自 **`agent_tokens.tenant_id`**）；**`Register`** 写主机快照并分配/确认 **`agent_id`**；**`Heartbeat`** 仅存活与版本门闸（不覆盖快照列）；**`LeaseJobs`** 与策略 **`bound_agent_id`** 双向收窄（策略创建必填绑定）。控制台 **`/execution/agent-tokens`**；**`deploy/scripts/bootstrap_demo_stack.py`** 在租户镜像成功后 **`GET`/`POST /api/v1/agent-tokens`** 写入共享卷 **`demo_stack_secrets`**；**`deploy/scripts/devault-agent-docker-entry.sh`**；**`scripts/e2e_grpc_register_heartbeat.py`**、**`deploy/iac/examples/curl-agent-token.sh`**；**`scripts/verify_console_openapi_contract.py`** 不再要求 **`PolicyOut.bound_agent_pool_id`**；文档站 **Agent 舰队 / 凭据生命周期 / quickstart / ER / gRPC**（含 **Mermaid 时序图**）、**`website/docs/admin/docker-compose.md`**。OpenSpec 变更目录 **`openspec/changes/refactor-agent-token-control-plane/`**。
- **§十六 P2（十六-11～十六-12）**：迁移 **`0017`** — 表 **`tenant_invitations`**；租户列 **`sso_oidc_issuer` / `sso_oidc_audience` / `sso_oidc_role_claim` / `sso_oidc_email_claim` / `sso_password_login_disabled` / `sso_jit_provisioning` / `sso_saml_entity_id` / `sso_saml_acs_url`**（SAML 仅登记，控制面不消费断言）。REST：**`POST`/`GET /api/v1/tenants/{tenant_id}/invitations`**、**`POST /api/v1/auth/invitations/accept`**（CSRF 豁免）；**`DEVAULT_INVITATION_TTL_HOURS`**、**`DEVAULT_INVITATION_LINK_BASE`**（可回退 **`DEVAULT_PASSWORD_RESET_LINK_BASE`**）。**`try_decode_tenant_oidc_bearer`**（**`src/devault/security/tenant_oidc.py`**）在全局 OIDC 之后解析 **Bearer**；**JIT** 可选 **`db.commit`**；**`console_user_password_login_blocked`** 在 **`POST /auth/login`** 上生效。控制台：**`/user/accept-invite`**、**`/overview/team-invitations`**（**`canInviteMembers`**）、平台 **租户** 表单 SSO 段；**`SessionTenantRow.sso_password_login_disabled`**。OpenAPI 闸门与 **`tests/test_sso_policy_unit.py`**、**`tests/test_invitation_accept_api.py`**。文档：**`docs-old/enterprise-backlog.md`**、**`enterprise-backlog-completed-archive.md`**、**`website/docs/admin/configuration.md`**、**`website/docs/guides/web-console.md`**、**`website/docs/user/web-console.md`**、**`website/docs/admin/tenants-and-rbac.md`**、**`console/README.md`**。
- **§十六 P1（十六-07～十六-10）**：迁移 **`0016`** — **`tenants.require_mfa_for_admins`**；**`console_users.totp_secret` / `totp_confirmed_at`**；**`password_reset_tokens`**。依赖 **`pyotp`**。REST：**`POST /api/v1/auth/session/refresh`**、**`register`**（**`DEVAULT_CONSOLE_SELF_REGISTRATION_ENABLED`**）、**`password-reset/request|confirm`**、**`mfa/verify`**、**`mfa/enroll/start|confirm`**；**`AuthSessionOut.needs_mfa`**；**`TenantOut` / `TenantPatch` / `SessionTenantRow`** 含 **`require_mfa_for_admins`**。服务：**`login_rate_limit`**、**`auth_audit`**、**`email_delivery`**（**`DEVAULT_SMTP_*`**；空 host 时 **INFO** 占位）、**`password_reset_flow`**、**`totp_util`**。会话载荷 **`mfa_verified`**；**`ensure_can_write`** 对 **tenant_user** 在未满足 MFA 时返回 **`mfa_required`**。控制台：**`/user/integration`**、**`/user/register`**、**`/user/reset-password`**；登录页 **TOTP 第二步**；平台 **租户** 表单开关；**Playwright** 冒烟改为 **`/user/integration`**。OpenAPI 闸门：**`verify_console_openapi_contract.py`** 增补 **auth** 路径、**`needs_mfa`**、**`TenantOut.require_mfa_for_admins`**。文档：**`docs-old/enterprise-backlog.md`**、**`enterprise-backlog-completed-archive.md`**、**`website/docs/guides/web-console.md`**、**`website/docs/user/web-console.md`**、**`website/docs/admin/configuration.md`**、**`website/docs/trust/api-access.md`**、**`console/README.md`**。
- **§十六 P0（十六-01～十六-06）**：**`console_users` / `tenant_memberships`**（Alembic **`0015`**）；**Argon2id**（**`argon2-cffi`**）；**`devault-admin create-console-user`**；**Redis** 会话（**`devault.security.http_session_store`**）与 **`DEVAULT_SESSION_*` / `DEVAULT_CSRF_*`**；**`get_auth_context`** **Cookie 优先**、**`AuthContext.principal_kind`**（**platform / tenant_user**）及 **`ensure_admin`** 平台闸门；**`CsrfProtectionMiddleware`**；**`GET/POST /api/v1/auth/csrf|login|logout`**；**`AuthSessionOut`** 扩展 **`user_id` / `email` / `tenants` / `principal_kind`**；**`console/`** **`credentials: 'include'`**、登录 **密码**（**P1** 起 **API Token** 迁至 **`/user/integration`**）、**`X-CSRF-Token`** 拦截器；**Playwright**（**P1** 起）打开 **`/user/integration`**。文档：**`docs-old/enterprise-backlog.md`**、**`website/docs/guides/web-console.md`**、**`website/docs/user/web-console.md`**。
- **十五-22～二十四**：**Playwright** 控制台冒烟（**`console/e2e/smoke.spec.ts`**、**`playwright.config.ts`**、**`npm run test:e2e`**）；CI 工作流 **`.github/workflows/console-e2e.yml`** 与 **`deploy/docker-compose.console-e2e.yml`**（源码构建 API + **`Dockerfile.console`**）。**十五-23**：**`GET /api/v1/jobs`** 增加 **`kind` / `status`** 查询（**`JobKind` / `JobStatus`**）；作业中心 **`ProTable`** 传参；**`tests/test_openapi_jobs_list_filters.py`**；**`verify_console_openapi_contract.py`** 校验 query。**十五-24**：**`/backup/run`** 三步 **Steps** 向导；工作台 **指标与看板**（**/metrics**、可选 **`UMI_APP_GRAFANA_URL`**，见 **`console/.env.example`**）。
- **十五-19～二十一**：**下线 Jinja `/ui`**：删除 **`src/devault/api/routes/ui.py`**、**`src/devault/web/templates/`** 与 **`deps.py`** 中 **`verify_ui_basic_auth` / `get_effective_tenant_ui` / Cookie 租户`**；**`jinja2`** 自运行依赖移除。策略路径 allowlist 合并/拆分迁至 **`src/devault/services/policy_paths_form_merge.py`**（测试同步）。**CI**：**`scripts/verify_console_openapi_contract.py`**（OpenAPI 无 **`/ui`**、**`JobOut`/`PolicyOut`** 关键字段）+ **`console/`** **`npm run codegen`** / **`npm run build`**。**交付**：**`deploy/Dockerfile.console`**（Python 导出 OpenAPI → Node 构建 → nginx）；**`deploy/docker-compose.yml`** **`console`** 改为 **`build`**；**Helm** **`console.enabled`** 时渲染 **Deployment/Service/ConfigMap** 与 Ingress 上 **`/api`**、**`/docs`** 等指向 API、**`/`** 指向控制台。文档：**`website/docs/guides/web-console.md`**、**`user/web-console.md`**、**`CONTRIBUTING.md`**、**PR 模板**、**`docs-old/enterprise-backlog.md`**。
- **十五-11～十八**：**`console/`** 在五大分组下落地与 **`/api/v1/*`** 对等的页面：**`/backup/jobs`**（`ProTable`、类型/状态筛选、取消/重试、详情抽屉含 **`config_snapshot`** / **`result_meta`** / hostname）、**`/backup/policies`** 与 **Tab 表单**（**`FileBackupConfigV1`**、执行绑定、内嵌 **`/schedules`** 按 **`policy_id`**）、**`/backup/run`**、**`/backup/precheck`**、**`/backup/artifacts`**（offset 分页、恢复/演练二次确认 **`RESTORE`** / **`DRILL`**、管理员 **Legal hold**）、**`/compliance/schedules`** 与 **`/compliance/restore-drill-schedules`** 全 CRUD、**`/execution/tenant-agents`**、**`/execution/agent-pools`**（成员 **PUT**）、**`/execution/fleet`**（详情、Enrollment **PUT**、**`revoke-grpc-sessions`** 强确认 **`REVOKE`**）、**`/platform/tenants`**（**`TenantPatch`**）；**`typings.d.ts`** 扩展 **`JobOut`** 等；**`config/config.ts`** 嵌套路由；**`workbench`** 链至作业中心。文档：**`docs-old/enterprise-backlog.md`** 索引与 **§15.2**、**`console/README.md`**、**`website/docs/guides/web-console.md`**、**`website/docs/user/web-console.md`**。
- **十五-01**：**`GET /api/v1/auth/session`**（**`AuthSessionOut`**：`role`、`principal_label`、`allowed_tenant_ids`），供 Ant Design Pro **`getInitialState`** / 租户选择器与 RBAC 对齐现有 **`AuthContext`**；测试 **`tests/test_auth_session_api.py`**。
- **十五-02**：**`console/`** 目录（**`openapi-typescript`**）、**`scripts/export_openapi_json.py`**、OpenAPI 类型（现 **`src/openapi/api-types.d.ts`**）与 **`console/README.md`**；CI 控制台校验见 **十五-03～06** 条（现为 **`npm run build`**）。
- **十五-03～06**：**`console/`** 升级为 **Umi 4（`@umijs/max`）+ Ant Design Pro 组件**（中文 **`antd` locale**、主题基线、侧栏布局）；**`request` 拦截器**（Bearer、**`X-DeVault-Tenant-Id`**、401 跳转登录、403 **`message`**）；**`getInitialState` + `access.ts`**（**`canAdmin` / `canWrite` / `isAuditor`**）；**`/user/login`** Bearer 表单（**`localStorage`**，非 HTTP Basic）。OpenAPI 类型迁至 **`src/openapi/`**；CI 改为 **`npm run build`** 校验前端。
- **十五-07～10**：顶栏 **`TenantSwitcher`**（**`GET /api/v1/tenants`** + **`localStorage`** 持久化租户 UUID，替代 Jinja Cookie 路径）；开发 **`proxy`** 扩展 **`/docs`**、**`/metrics`**、**`/version`**、**`/healthz`**；**`deploy/nginx/console-spa.conf`** 与 Compose **`console`** 服务（**`--profile with-console`**）；侧栏 **五大分组** + 顶栏 **环境标签**（**`UMI_APP_ENV_LABEL`**）与 **帮助** 下拉；**工作台**（**`/version`** + 最近失败/进行中 **Jobs**）。文档与根 **README** 目录表更新。
...
- **Agent 登记 / 池 / Redis gRPC 会话（主路径下线）**：**`agent_enrollments`** 与 **`PUT`/`GET …/enrollment`**；**`agent_pools`** / **`agent_pool_members`** 与 **`/api/v1/agent-pools`** 及控制台 Agent 池页；**`src/devault/security/agent_grpc_session.py`** 与 **`POST …/revoke-grpc-sessions`**；**`registration_secret`** 主路径及 **`DEVAULT_AGENT_ID`** Compose 默认等（详见仓库 OpenSpec **`refactor-agent-token-control-plane`**）。
