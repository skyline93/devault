...
默认 Compose **不为 agent 设置** **`DEVAULT_API_TOKEN`**：启动时通过 **Register**（**`DEVAULT_GRPC_REGISTRATION_SECRET`** 与 **api** 相同）向 Redis 领取 **绑定该 Agent id** 的 gRPC Bearer。若需跳过 Register、固定使用共享令牌，可为 agent 显式注入 **`DEVAULT_API_TOKEN`**。
