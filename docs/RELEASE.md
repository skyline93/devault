...
> 复制本页为「版本 x.y.z 发版笔记」或在 PR 描述中逐项勾选。与 **`website/docs/engineering/releasing.md`**、`**scripts/bump_release.py**` 及 **`CHANGELOG.md`** 配合使用。

## 1. 升级顺序

- [ ] **确认依赖方向**：先升级 **控制面**（API + gRPC + 迁移），再滚动 **Agent**；或按 `docs/compatibility.json` 中矩阵说明的例外执行。
- [ ] **数据库**：在控制面镜像上线前或按零停机流程执行 **`alembic upgrade head`**（见 `website/docs/admin/database-migrations.md`）。
...
- [ ] **TLS / 网关**：生产 gRPC 走 TLS 或 Envoy 等（见 `website/docs/trust/tls-and-gateway.md`）。
...
- [ ] （建议）在默认分支手动运行 **`.github/workflows/e2e-version-matrix.yml`**（Compose + **Register** / **Heartbeat** 冒烟）。若需「当前控制面 + 上一 MINOR Agent」等交叉行，先将 **`docs/compatibility.json`** · **`ci_e2e.previous_minor_git_ref`** 设为可解析的 git ref（例如 **`v0.3.0`** tag）并合并后再跑；详见 **`website/docs/engineering/compatibility.md`**。
