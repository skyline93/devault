...
- **Agent → 网关 → gRPC 服务**：对外统一控制入口；开发环境可省略网关，Agent 直连 `DEVAULT_GRPC_TARGET`。
...
| Agent gRPC | 与上者**同一进程**内 `DEVAULT_GRPC_LISTEN`（如 `:50051`） |
| 调度器 | **`devault-scheduler`**，**不**执行 `alembic` |
| Agent | **`devault-agent`**，配置 gRPC 目标与 `DEVAULT_ALLOWED_PATH_PREFIXES` |
| 数据库迁移 | 默认仅 **api** 启动时 `alembic upgrade head`（见 [数据库迁移](../admin/database-migrations.md)） |
