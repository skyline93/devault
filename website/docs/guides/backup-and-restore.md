...
首启且未配置 `DEVAULT_API_TOKEN` 时，Agent 可先调用 **`Register`** 用一次性密钥换取 Bearer（见 `agent.proto` 注释）；日常循环中还会发送 **`Heartbeat`**。大对象分片、恢复侧流式与重试见 [大对象与恢复](../storage/large-objects.md) 与 [存储调优](../storage/tuning.md)。**Bundle 加密**：策略 **`encrypt_artifacts`**；密钥可为 **静态 `DEVAULT_ARTIFACT_ENCRYPTION_KEY`** 或 **KMS 信封**（Agent 侧 **`kms:GenerateDataKey` / `kms:Decrypt`**，见 [Artifact 静态加密](../security/artifact-encryption.md)）。**对象存储**可按租户使用客户桶与 AssumeRole（BYOB），见 [租户模型](../reference/tenants.md) 与 [对象存储模型](../storage/object-store-model.md)。
