...
Artifact 落在平台运维的 **S3 兼容**集群，通过前缀或桶策略做多租户。**Agent** 仅有短时 PUT/GET 授权；控制面持久化 key、checksum、保留元数据。存储连接见 **`storage_profiles`**：[租户与访问控制](../admin/tenants-and-rbac.md) 与 [对象存储模型](../storage/object-store-model.md)。
