...
from devault.services.tenant_backup_allowlist import list_registered_agent_ids_for_tenant
...
def tenant_scoped_agents_for_tenant(db: Session, tenant_id: uuid.UUID) -> list[TenantScopedAgentOut]:
    """Registered Agent instances for ``tenant_id``."""
    registered_ids = list_registered_agent_ids_for_tenant(db, tenant_id)
    ...
    for aid in registered_ids:
        ...
        if edge is None:
            continue
        base = edge_agent_to_out(edge)
        out.append(
            TenantScopedAgentOut(
                id=base.id,
                agent_token_id=base.agent_token_id,
