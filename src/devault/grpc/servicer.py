    ...
    def RequestStorageGrant(
        ...
        try:
            ...
            if request.intent == agent_pb2.STORAGE_INTENT_READ:
                ...
                if aid is None:
                    context.abort(grpc.StatusCode.FAILED_PRECONDITION, "restore artifact missing")
            ...
            if request.success:
                if job.kind == JobKind.BACKUP.value:
                    ...
                    if not bundle_key or not manifest_key:
                        context.abort(grpc.StatusCode.INVALID_ARGUMENT, "bundle/manifest keys required")
                    if not storage.exists(bundle_key) or not storage.exists(manifest_key):
                        context.abort(grpc.StatusCode.FAILED_PRECONDITION, "artifact objects missing")
                    art = Artifact(
                        job_id=job.id,
